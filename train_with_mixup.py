...
def val(args: argparse.Namespace, dataloaders, dataset_sizes, class_to_idx, model, criterion, epoch, step):
    device = args.device
    logger = args.logger
    # Validation phase
    model.eval()
    ...
    with torch.no_grad():
        for inputs, labels in dataloaders["val"]:
            ...
            val_predict_prob.append(softmax_output.cpu().detach().numpy()[:, val_fake_id])
    ...
    weight_path = os.path.join(args.weight_dir, f"{val_epoch_acc:.4f}_epoch{epoch}_step{step}.pt")
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in dataloaders["train"]:
            ...
            if step == 1 or step % 1000 == 0:
                val(args, dataloaders, dataset_sizes, class_to_idx, model, criterion, epoch, step)
            scheduler.step()
